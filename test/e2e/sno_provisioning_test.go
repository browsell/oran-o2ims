...
var _ = Describe("SNO End-to-end ProvisioningRequestReconcile with metal3 plugin", Ordered, Label("sno-provisioning"), func() {
	...
	for _, bmhData := range testutils.TestBMHs {
		bmh := testutils.CreateBareMetalHost(testutils.BMHData{
			Name:           bmhData.Name,
			Namespace:      constants.DefaultNamespace,
			MacAddress:     bmhData.MacAddress,
		...
		hwTemplateCRs = append(hwTemplateCRs, bmh, hwData, bmcSecret)
	}
...
defaultHugepagesSize: "1G"`,
	...
	AfterAll(func() {
		...
		// Delete BMH-related resources first
		for _, bmhData := range testutils.TestBMHs {
			bmh := &metal3v1alpha1.BareMetalHost{}
			if err := K8SClient.Get(testCtx, types.NamespacedName{
				Name: bmhData.Name, Namespace: constants.DefaultNamespace}, bmh); err == nil {
				_ = K8SClient.Delete(testCtx, bmh)
			}
			hwData := &metal3v1alpha1.HardwareData{}
			if err := K8SClient.Get(testCtx, types.NamespacedName{
				Name: bmhData.Name, Namespace: constants.DefaultNamespace}, hwData); err == nil {
				_ = K8SClient.Delete(testCtx, hwData)
			}
			secret := &corev1.Secret{}
			if err := K8SClient.Get(testCtx, types.NamespacedName{
				Name: fmt.Sprintf("%s-bmc-secret", bmhData.Name), Namespace: constants.DefaultNamespace}, secret); err == nil {
				_ = K8SClient.Delete(testCtx, secret)
			}
		}

		// Delete ProvisioningRequests.
		// Strip finalizers first: the PR controller's finalizer waits for cluster namespace
		...
		for _, name := range []string{"cluster-1", "cluster-2"} {
			...
			if err := K8SClient.Get(testCtx, types.NamespacedName{Name: name}, pr); err == nil {
				pr.Finalizers = nil
				_ = K8SClient.Update(testCtx, pr)
				_ = K8SClient.Delete(testCtx, pr)
			}
		}
		...
		// Delete ConfigMaps and Secrets created in test namespaces.
		for _, name := range []string{ciDefaultsCmIncomplete, ciDefaultsCmComplete, ptDefaultsCm} {
			cm := &corev1.ConfigMap{}
			if err := K8SClient.Get(testCtx, types.NamespacedName{Name: name, Namespace: ctNamespace}, cm); err == nil {
				_ = K8SClient.Delete(testCtx, cm)
			}
		}
		secret := &corev1.Secret{}
		if err := K8SClient.Get(testCtx, types.NamespacedName{Name: "pull-secret", Namespace: ctNamespace}, secret); err == nil {
			_ = K8SClient.Delete(testCtx, secret)
		}
	})
