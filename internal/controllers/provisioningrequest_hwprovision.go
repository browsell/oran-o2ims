...
func (t *provisioningRequestReconcilerTask) buildNodeAllocationRequest(
	...
	for _, ngRaw := range nodeGroupDataSlice {
		...
		if hwProfile == "" {
			return nil, fmt.Errorf("no hwProfile specified for nodeGroup %q: "+
				"provide it via hwMgmtParameters.nodeGroupData or in the hwMgmtDefaults ConfigMap", name)
		}
...
func (t *provisioningRequestReconcilerTask) handleRenderHardwareTemplate(ctx context.Context,
	...
	// Get the hardware plugin reference from the merged data, defaulting to metal3
	hwPluginRef := hwmgmtv1alpha1.DefaultHardwarePluginRef
	if ref, ok := hwMgmtData["hardwarePluginRef"].(string); ok && ref != "" {
		hwPluginRef = ref
	}
