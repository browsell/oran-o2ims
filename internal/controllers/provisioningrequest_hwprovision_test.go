...
var _ = Describe("handleRenderHardwareTemplate", func() {
	...
	It("returns an error when hwMgmtData is nil and HardwarePlugin not found", func() {
		// hwMgmtData is nil, defaulting to metal3 plugin which doesn't exist
		task.clusterInput = &clusterInput{
			hwMgmtData: map[string]any{},
		}
		...
		Expect(err).To(HaveOccurred())
		Expect(nodeAllocationRequest).To(BeNil())
		Expect(err.Error()).To(ContainSubstring("could not find specified HardwarePlugin"))
	})
})
...
var _ = Describe("buildNodeAllocationRequest", func() {
	...
	It("returns error when no hwProfile is provided", func() {
		clusterInstance := &unstructured.Unstructured{}
		clusterInstance.Object = map[string]interface{}{
			"spec": map[string]interface{}{
				"nodes": []interface{}{
		...
		task.clusterInput = &clusterInput{
			hwMgmtData: map[string]any{
				"nodeGroupData": []any{
					map[string]any{"name": "controller", "role": "master", "resourcePoolId": "pool-1"},
					// No hwProfile set
				},
			},
		}
		...
		Expect(err).To(HaveOccurred())
		Expect(nar).To(BeNil())
		Expect(err.Error()).To(ContainSubstring("no hwProfile specified for nodeGroup"))
	})
})
...
var _ = Describe("applyNodeConfiguration", func() {
	...
	It("returns error when hardware provisioning is skipped", func() {
		...
		task.ctDetails.templates.HwMgmtDefaults = ""
		...
		template = &provisioningv1alpha1.ClusterTemplate{
			...
			Spec: provisioningv1alpha1.ClusterTemplateSpec{
				...
				Templates: provisioningv1alpha1.Templates{
					...
					HwMgmtDefaults:          "test-hw-template-update-after-failure",
				},
			},
			Status: provisioningv1alpha1.ClusterTemplateStatus{
				Conditions: []metav1.Condition{
