...
// The HardwarePluginRef is stored in the hwMgmtDefaults ConfigMap referenced by the ClusterTemplate.
func GetHardwarePluginRefFromProvisioningRequest(ctx context.Context, c client.Client,
	...
	hwMgmtDefaultsCm := clusterTemplate.Spec.Templates.HwMgmtDefaults
	if hwMgmtDefaultsCm == "" {
		return "", fmt.Errorf("missing hwMgmtDefaults reference in ClusterTemplate")
	}

	cm, err := GetConfigmap(ctx, c, hwMgmtDefaultsCm, clusterTemplate.Namespace)
	if err != nil {
		return "", fmt.Errorf("failed to get hwMgmtDefaults ConfigMap %s: %w", hwMgmtDefaultsCm, err)
	}

	hwMgmtDefaults, err := ExtractTemplateDataFromConfigMap[map[string]any](cm, HwMgmtDefaultsConfigmapKey)
	if err != nil {
		return "", fmt.Errorf("failed to extract hwMgmt defaults: %w", err)
	}

	if ref, ok := hwMgmtDefaults["hardwarePluginRef"].(string); ok && ref != "" {
		return ref, nil
	}

	return hwmgmtv1alpha1.DefaultHardwarePluginRef, nil
}
